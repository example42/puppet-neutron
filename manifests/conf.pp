...
# = Define: neutron::conf
#
# With this define you can manage any neutron configuration file
#
# == Parameters
#
# [*template*]
...
#   Example: template => 'site/neutron/my.conf.erb',
#
# [*content*]
...
#   Example: source => 'puppet:///site/neutron/my.conf',
#
# [*ensure*]
...
define neutron::conf (
  ...
  $manage_path    = pickx($path, "${neutron::config_dir_path}/${name}")
  $manage_content = default_content($content, $template)
  $manage_mode    = pickx($mode, $neutron::config_file_mode)
  $manage_owner   = pickx($owner, $neutron::config_file_owner)
  $manage_group   = pickx($group, $neutron::config_file_group)
  $manage_require = pickx($config_file_require, $neutron::config_file_require)
  $manage_notify  = $config_file_notify ? {
    'class_default' => $neutron::manage_config_file_notify,
    default         => $config_file_notify,
  }

  file { "neutron_conf_${name}":
    ensure  => $ensure,
    source  => $source,
